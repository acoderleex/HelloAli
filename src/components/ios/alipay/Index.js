...
const AlipayView = React.createClass({
  ...
  quickPress(index){
    if (index=== 0) {
      this.navigatorPush('appQuick','ScanQuick',AppInfoView,{backButtonText:'QuickEntry'});
    }
  },
    ...
    if (route.name === 'alipay-index') {
      return (
        <TouchableOpacity style={{flex:1,alignItems:'center'}} onPress={
            () =>  navigator.push(assign({
                title: "test",
                name: "NAMEtEST",
                component: AppMoreView,
              },{backButtonText:"张帆吧"}))
        }>
  ...
  RightButton(route, navigator, index, navState) {
    ...
    return (
        <NavBar.RightButton styles={styles}>
          ...
          <TouchableOpacity style={{flex:1,alignItems:'center'}} onPress={()=>navigator.push(assign(
            {
                title: "Ledt",
                name: "Ttttt",
          ...
          <Text key="topBarIcon" style={styles.topBarIcon}>{personIcon}&nbsp;&nbsp;{addIcon}</Text>
          </TouchableOpacity>
          <Text key="addTopBarIcon" style={styles.topBarIcon}>{searchIcon}&nbsp;&nbsp;</Text>
          </View>
        </NavBar.RightButton>
    );
  },
  ...
  render() {
    return (
        <Navigator initialRoute={{name: 'alipay-index', component: AlipayView}}
                   configureScene={() => { return Navigator.SceneConfigs.HorizontalSwipeJumpFromRight }}
                   navigationBar={
